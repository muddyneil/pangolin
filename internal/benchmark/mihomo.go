...
func NewMihomo(ctx context.Context, core string, config []byte, geoIPPath string) (*Mihomo, error) {
	if strings.TrimSpace(core) == "" {
		return nil, errors.New("mihomo path is empty")
	}
	dir, err := os.MkdirTemp("", "pangolin-benchmark-")
	if err != nil {
		return nil, fmt.Errorf("failed to create benchmark temporary directory: %w", err)
	}
	keep := false
	defer func() {
		if !keep {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to write benchmark port configuration: %w", err)
	}
	if err := os.WriteFile(configPath, config, 0600); err != nil {
		return nil, fmt.Errorf("failed to write benchmark configuration: %w", err)
	}
	if geoIPPath != "" {
		...
		if err := os.WriteFile(filepath.Join(dir, "geoip.metadb"), data, 0600); err != nil {
			return nil, fmt.Errorf("failed to copy GeoIP database: %w", err)
		}
	}
...
func ValidateConfig(config []byte, core string) error {
	if strings.TrimSpace(core) == "" {
		return errors.New("mihomo path is empty")
	}
	dir, err := os.MkdirTemp("", "pangolin-validate-")
	if err != nil {
		return fmt.Errorf("failed to create validation temporary directory: %w", err)
	}
	defer os.RemoveAll(dir)
	configPath := filepath.Join(dir, "config.yaml")
	if err := os.WriteFile(configPath, config, 0600); err != nil {
		return fmt.Errorf("failed to write validation configuration: %w", err)
	}
...
// ConfigRejectedError marks a failed `mihomo -t` with captured output so the
// benchmark can distinguish a candidate-specific rejection from an engine
// failure.
...
func checkConfig(ctx context.Context, core, configPath string) error {
	...
	if !errors.As(err, &exitErr) {
		return fmt.Errorf("Mihomo configuration check process failed: %w", err)
	}
	text := summarize(output)
	return &ConfigRejectedError{Err: fmt.Errorf("Mihomo configuration check failed: %w: %s", err, text), Output: text}
}
...
func (m *Mihomo) waitReady(ctx context.Context) error {
	...
	for {
		...
		case <-ctx.Done():
			return ctx.Err()
		case <-deadline.C:
			return fmt.Errorf("Mihomo controller startup timed out: %s", m.Log())
		case <-ticker.C:
...
func withPorts(data []byte, controlPort, mixedPort int) ([]byte, error) {
	...
	if err := json.Unmarshal(data, &config); err != nil {
		return nil, fmt.Errorf("failed to parse benchmark configuration: %w", err)
	}
...
func freePort() (int, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("failed to allocate localhost port: %w", err)
	}
