...
func fields(m map[string]any) map[string]any {
	...
	for key, value := range m {
		...
		if key == "ws-opts" || key == "http-opts" || key == "h2-opts" || key == "grpc-opts" || key == "plugin-opts" || key == "reality-opts" {
...
func filled(m map[string]any, key string) bool {
	value, ok := m[key]
